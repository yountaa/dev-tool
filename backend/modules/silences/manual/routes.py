...
from .builder import build_manual
from logging_setup import event

router = APIRouter(prefix="/{env}/manual", tags=["silences:manual"])
log = logging.getLogger("silences.api")


@router.post("")
async def create_manual(env: str, req: ManualRequest, user: str = Depends(current_user)):
    ...
    dup = save_hub.find_duplicate("manual", env, payload)
    if dup is not None:
        raise HTTPException(409, f"Такое правило уже есть: «{dup.payload.get('name') or dup.id}»")
    cfg_id = uuid.uuid4().hex[:8]
    am_id = ""
    try:
        body = build_manual(req)
        body["comment"] = tag_comment("manual", cfg_id, req.name, body["comment"])  # метим для связи с правилом
        am_id = await client.create_silence(env, body)
    except AlertmanagerError:
        pass  # AM недоступен — поставит шедулер позже
    cfg = save_hub.save("manual", env, payload, cfg_id=cfg_id, am_id=am_id, actor=actor, action="создал")
    event(log, "rule.created", env=env, kind="manual", id=cfg.id, name=req.name, placed=bool(am_id))
    return {"silence_id": am_id, "config_id": cfg.id, "placed": bool(am_id)}


@router.get("")
def list_manual(env: str):
    """Показать все разовые конфиги этого окружения (история из git)."""
    require_env(env)
    return save_hub.list_configs("manual", env)
